        ...
        <div className="  lg:w-1/2 lg:border-l-2  p-10 h-full">
          ...
          <div className="flex flex-wrap items-start gap-5 mt-5 h-[90%] overflow-auto ">
            {task.map(function(elem, idx){
              return <div key={idx} className="flex justify-between flex-col items-start relative h-52 w-40 text-black bg-cover rounded-xl pt-9 pb-4 px-4 bg-[url('https://static.vecteezy.com/system/resources/thumbnails/010/793/873/small/a-lined-note-paper-covered-with-transparent-tape-on-a-yellow-background-with-a-white-checkered-pattern-free-png.png')]">
                <div>
                ...
                <p className="mt-2 leading-tight font-medium text-gray-700">{elem.details}</p>
                 ...
                 <button onClick={()=>{
                  deleteNote(idx)
                 }} className="w-full bg-red-600 py-1 text-xs rounded font-bold cursor-pointer active:scale-95 text-white">Delete</button>
              </div>
            })}
